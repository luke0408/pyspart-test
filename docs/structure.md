...
- **grafana**: 집계된 KPI 데이터를 시각화하는 대시보드 도구입니다. 미리 정의된 대시보드를 통해 비즈니스 지표를 한눈에 파악할 수 있습니다.
...
읽기 경로는 요약 테이블 중심입니다. API는 `daily_traffic_summary`와 `daily_conversion_funnel`의 집계 결과를 조회해 KPI 응답을 제공하고, Grafana도 같은 요약 테이블을 데이터 소스로 사용해 대시보드를 구성합니다.
...
2. Docker 서비스 부팅
   - `postgres`, `api`, `spark`, `grafana`를 함께 시작합니다.
...
4. Grafana 읽기 전용 역할 bootstrap
   - Grafana 데이터소스가 KPI 요약 테이블을 읽을 수 있도록 readonly role을 생성/갱신합니다.
   - 명령: `docker compose exec api python scripts/bootstrap_grafana_readonly_role.py`
5. 결정론적 시드 데이터 적재
...
검증은 수동 확인(SQL/API/Grafana)과 자동 확인(pytest + Grafana 검증 스크립트)을 함께 수행합니다.
...
4. 자동 검증
   - 테스트 스위트와 Grafana 검증 스크립트를 실행해 회귀 여부를 확인합니다.
   - 명령:
...
- 생성된 자산 및 프로비저닝 파일: `grafana/**/*.json`, `grafana/provisioning/**/*.yml`
